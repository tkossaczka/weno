...
def WENO6(u,m,l,e,omegas):
    uu=u[:,l-1];

    fluxp0 = (  uu[1:m+1-5] - 3*uu[2:m+1-4] - 9*uu[3:m+1-3]  + 11*uu[4:m+1-2])/12;
    fluxp1 = (   uu[2:m+1-4] - 15*uu[3:m+1-3]+ 15*uu[4:m+1-2] - uu[5:m+1-1]  )/12;
    fluxp2 = (-11*uu[3:m+1-3]+ 9*uu[4:m+1-2] + 3*uu[5:m+1-1] - uu[6:m+1-0] )/12;

    fluxn0 = (  uu[0:m+1-6] - 3*uu[1:m+1-5] - 9*uu[2:m+1-4] + 11*uu[3:m+1-3])/12;
    fluxn1 = (   uu[1:m+1-5] - 15*uu[2:m+1-4] + 15*uu[3:m+1-3]- uu[4:m+1-2]  )/12;
    fluxn2 = (-11*uu[2:m+1-4] + 9*uu[3:m+1-3]  + 3*uu[4:m+1-2] - uu[5:m+1-1] )/12;
