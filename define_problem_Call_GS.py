...
class Call_option_GS(Call_option):
    ...
    def __compute_n_t_h_x_time(self):
        ...
        n = np.ceil(np.max((theta * sigma**2) / (0.8 * (h**2) * xx**2)))
        n = int(n)
        t = theta / n
        ...
        for k in range(0, n + 1):
            for j in range(0, m + 1):
                Call[j, k] = S[j] * norm.cdf(
                    (np.log(S[j] / E) + (rate + (sigma ** 2) / 2) * (T - tt[k])) / (
                                sigma * np.sqrt(T - tt[k]))) - E * np.exp(-rate * (T - tt[k])) * norm.cdf(
