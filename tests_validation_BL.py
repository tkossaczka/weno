...
for i in range(8000):
    if not (i % test_modulo):
        print(i)
        train_model = torch.load('C:/Users/Tatiana/Desktop/Research/Research_ML_WENO/Buckley_Leverett_CD_Test/Models/Model_42/{}.pt'.format(i))
        loss_test = []
        for kk in range(rng):
...
aa = np.linspace(0, 7900, 80)
my_xticks = aa.tolist()
plt.xticks(np.arange(0, 7900, 400))
plt.plot(my_xticks,norm_losses)
# plt.savefig("BL_validation.pdf", bbox_inches='tight')

# np.save("C:/Users/Tatiana/Desktop/Research/Research_ML_WENO/PME_Test/Models/Model_37/all_loss_test.npy",all_loss_test)
# np.save("C:/Users/Tatiana/Desktop/Research/Research_ML_WENO/Buckley_Leverett_CD_Test/Models/Model_42/all_loss_test.npy",all_loss_test)
